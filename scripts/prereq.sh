...
# ----- Config -----
BUCKET_NAME=${1:-multiagent-lab}
INFRA_STACK_NAME=${2:-MultiAgentStackInfra}
...
aws s3api put-object --bucket "$FULL_BUCKET_NAME" --key "$S3_KEY" --body "$PROJECT_ROOT/$ZIP_FILE" --expected-bucket-owner "$ACCOUNT_ID"
...
deploy_stack "$INFRA_STACK_NAME" "$PROJECT_ROOT/$INFRA_TEMPLATE_FILE" --parameter-overrides LambdaS3Bucket="$FULL_BUCKET_NAME" LambdaS3Key="$S3_KEY" LayerS3Key="$S3_LAYER_KEY"
infra_exit_code=$?

echo "🔧 Starting deployment of Cognito stack..."
deploy_stack "$COGNITO_STACK_NAME" "$PROJECT_ROOT/$COGNITO_TEMPLATE_FILE"
cognito_exit_code=$?
