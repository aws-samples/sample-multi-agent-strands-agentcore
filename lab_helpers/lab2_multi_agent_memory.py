...
def delete_memory(memory_hook):
    try:
        ...
        ssm_client.delete_parameter(Name="/app/reinvent/agentcore/memory_id")
    except Exception:
        pass
...
# Alias for backward compatibility with lab4 notebook
def create_or_get_multi_agent_memory():
    """Alias for create_or_get_memory_resource for multi-agent compatibility"""
    return create_or_get_memory_resource()


# Alias class for backward compatibility with lab4 notebook  
class MultiAgentMemoryHooks(CustomerSupportMemoryHooks):
    """Alias for CustomerSupportMemoryHooks for multi-agent compatibility"""
    
    def __init__(self, memory_id, client, actor_id, session_id, agent_type=None):
        # Ignore the agent_type parameter and call parent constructor
        super().__init__(memory_id, client, actor_id, session_id)
